...
if($antigo != $cpf){
	$query = $pdo->query("SELECT * FROM motoristas where cpf = '$cpf' ");
...
if($antigo2 != $email){
	$query = $pdo->query("SELECT * FROM motoristas where email = '$email' ");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){
...
if($id == ""){
	$res = $pdo->prepare("INSERT INTO motoristas SET nome = :nome, cpf = :cpf, email = :email, endereco = :endereco, telefone = :telefone");	

	$res2 = $pdo->prepare("INSERT INTO usuarios SET nome = :nome, cpf = :cpf, email = :email, senha = :senha, nivel = :nivel");	
	$res2->bindValue(":senha", '123');
	$res2->bindValue(":nivel", 'motoristas');

}else{
	$res = $pdo->prepare("UPDATE motoristas SET nome = :nome, cpf = :cpf, email = :email, endereco = :endereco, telefone = :telefone WHERE id = '$id'");
